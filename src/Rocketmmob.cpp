...
void RocketMob::Update(float deltaTime, sf::View &camera, Player &player)
{
    ...
    if (!moving)
    {
        rocketLauncher.Fire(RocketBullet{
            rocketLauncher.GetProjectileTexture(),
            rocketMobSprite.getPosition(),
            (player.GetSprite().getPosition() - rocketMobSprite.getPosition()),
            rocketDamage,
            rocketVelocity
        });
    }
    ...
    {
        // direct hit gonna make explosion
        if (!rocket.HasExploded() && player.GetBounds().findIntersection(rocket.GetBounds()))
        {
            rocket.Explode();
        }

        // damge if player in radius of explosion 
        if (rocket.HasExploded() && player.GetBounds().findIntersection(rocket.GetBounds()))
        {
